...
class App.Controllers.Musos.Weekends extends App.Controllers.Stackable
  elements:
    "header" : "header"
    "header h2" : "title"
  ...
  init: ->
    ...
    @weekends = new App.Controllers.Infinite el: @$(".weekends"), items: App.Controllers.Musos.Weekend
    @weekends.bind "change", @change
    @weekends.bind "changing", @changing
    @route "/musos", @home
    @route "/musos/:year/:month/:day", (params) =>
  ...
  changing: (index) =>
    ...
    unless index is @index
      @path = "/musos/#{friday.format("%Y/%o/%d")}"
      if @index?
  ...
  change: (index) =>
    friday = @constructor.fromIndex index
    saturday = new Date(friday.getTime() + Date.DAY)
    @navigate "/musos/#{friday.format("%Y/%o/%d")}", false
